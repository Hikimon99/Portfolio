...
var doors2;
// var doors3;
var turret, bullets, enemy, bullet1, bullet3;
var uplook = 0;
var look = 0;
var graphics;
...
export default class Level4 extends Phaser.Scene {
  ...
  create (data) {
    ...
    var crack = this.add.sprite(1530, 300, 'crack').setScale(0.3);
    ...
    emitter0.setPosition(1530, 300);
    emitter0.setGravityY(500);

    var crack = this.add.sprite(250, 100, 'crack').setScale(0.3);
    ...
    emitter1.setPosition(250, 100);
    emitter1.setGravityY(500);
    //***********************END OF WATER*************************

    //Level number
    this.levelNumber = this.add.text(725, 20, 'Level 4', {
    ...
    platforms.create(1000, 500, 'ground').setScale(.5,.5).refreshBody();
    platforms.create(700, 400, 'ground').setScale(.5,.5).refreshBody();

    platforms.create(200, 300, 'ground').setScale(1.5,.5).refreshBody();
    platforms.collideWorldBounds = true;
    //*************************END OF PLATFORMS****************************
    ...
    //************************DOORS**************************
    //Add doors
    this.doors2 = this.physics.add.sprite(100, 250, 'greendoor-closed');
    ...
    // Add turrets
    this.turret1 = this.add.sprite(1300, 25, 'turret');
    this.turret1.setScale(1);
    this.turret1.setAngle(90);

    this.turret3 = this.add.sprite(700, 25, 'turret');
    ...
    //greendoor not adjusted
      ...
      this.player = this.physics.add.sprite(350, 150, "playersAv");
    }
    ...
    shields.create(1700, 865, 'shield').setScale(0.3).refreshBody();
    shields.create(1300, 765, 'shield').setScale(0.3).refreshBody();
    shields.create(1700, 665, 'shield').setScale(0.3).refreshBody();
    shields.create(1300, 565, 'shield').setScale(0.3).refreshBody();
    shields.create(700, 365, 'shield').setScale(0.3).refreshBody();
    ...
    if (this.haveSword && attackKey.isDown && spaceDown == false && time - lastSlash >= 700) {
      ...
      if (Phaser.Geom.Rectangle.Overlaps(this.rect, grect)) {
        ...
        this.hpChange(this.ghost, -1, this.ghost.hp, this.ghost.id);
        ...
        this.ghost.x = this.ghost.x - gha;
        this.ghost.y = this.ghost.y - ghb;
      }
...
//change HP based on gain or loss
  hpChange (thing, hpplus, thishp, id){
    ...
    if (thishp <= 0) {
      ...
      if(this.haveKey == false && this.spawn) {
        this.keyers = this.physics.add.sprite(300,250, "keyer");
  ...
  fillInvent(refill, identify){
    if( identify == 2 && this.numShield > 0){
      if (this.invent[1][2] == 0 && this.textisSet == false){
  ...
  collectkey() {
    ...
    this.inventKey.setScale(.3);
    this.inventKey.setScrollFactor(0, 0);
    this.doors2 = this.physics.add.sprite(100, 250, "greendoor-open");
    this.physics.add.collider(this.doors2, platforms);
    this.physics.add.overlap(this.player, this.doors2, this.sceneNext, null, this);
  }
